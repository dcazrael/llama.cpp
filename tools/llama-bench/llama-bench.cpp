...
static cmd_params parse_cmd_params(int argc, char ** argv) {
    ...
    for (int i = 1; i < argc; i++) {
        ...
        try {
            if (arg == "-h" || arg == "--help") {
                ...
                auto p = parse_int_range(argv[i]);
                params.n_moe_cache_slots.insert(params.n_moe_cache_slots.end(), p.begin(), p.end());
    ...
    if (params.n_moe_cache_slots.empty()) {
        params.n_moe_cache_slots = cmd_params_defaults.n_moe_cache_slots;
    }
